...
static inline void pit_set_reload(uint16_t reload_value) {
    port_write_u8(PIT_COMMAND, PIT_CHANNEL0_SEL | PIT_ACCESS_LOHI | PIT_MODE_RATE | PIT_MODE_BINARY);
    port_write_u8(PIT_CHANNEL0, reload_value & 0xFF);
    port_write_u8(PIT_CHANNEL0, (reload_value >> 8) & 0xFF);
}
...
void pit_sleep_us(uint64_t microseconds) {
    ...
    while(total_ticks > 0) {
        ...
        while(1) {
            port_write_u8(PIT_COMMAND, 0x00);
            uint8_t lo = port_read_u8(PIT_CHANNEL0);
            uint8_t hi = port_read_u8(PIT_CHANNEL0);
