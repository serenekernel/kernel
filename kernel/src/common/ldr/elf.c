...
bool elf_supported(const elf64_elf_header_t* elf_header) {
    ...
    printf("e_type: %u\n", elf_header->e_type);
    if(elf_header->e_type != ETYPE_EXEC) { return false; }
    return true;
}
...
void load_elf_exec(process_t* process, const elf64_elf_header_t* elf_header, elf64_elf_loader_info_t* loader_info) {
    ...
    for(uint16_t i = 0; i < elf_header->e_phnum; i++) {
        ...
        vm_memcpy(process->allocator, &kernel_allocator, (virt_addr_t) segment_vaddr, ((virt_addr_t) elf_header + phdr->p_offset), phdr->p_filesz);
        for(size_t i = 0; i < 16; i++) { printf("0x%x ", *(uint8_t*) ((virt_addr_t) elf_header + phdr->p_offset + i)); }
        printf("\n");
        bool prev = arch_disable_uap();
        for(size_t i = 0; i < 16; i++) { printf("0x%x ", *(uint8_t*) ((virt_addr_t) segment_vaddr + i)); }
        arch_restore_uap(prev);
        printf("\n");
        printf("memsz=%zu, filesz=%zu\n", phdr->p_memsz, phdr->p_filesz);
        if(phdr->p_memsz > phdr->p_filesz) {
            size_t bss_size = phdr->p_memsz - phdr->p_filesz;
            vm_memset(process->allocator, (virt_addr_t) (segment_vaddr + phdr->p_filesz), 0, bss_size);
        }

        vm_flags_t flags = VM_READ_ONLY;
        printf("load: segment_start=0x%llx, segment_end=0x%llx, segment_pages=%zu, flags=[R", segment_start, segment_end, segment_pages);
